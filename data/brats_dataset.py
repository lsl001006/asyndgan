...
class BratsDataset(BaseDataset):
    ...
    def __init__(self, opt, idx=None):
        ...
        self.is_test = False # set to True if need more data to train
    ...
    def build_pairs(self, dataset):
        ...
        for key in keys:
            ...
            # 从3d-dcm图像中提取出若干有效2d图像的[图像(list), 边缘(list), 分割label(list)]
            for i in range(label.shape[2]):
                
                if len(np.unique(label[:, :, i])) > 1: # when the seg label is not none
                    # pdb.set_trace()
                    slice_dcm = dcm[:, :, i] # the slice of brain in format of t2
                    # slice_dcm = slice_dcm * ((pow(2, 16) - 1) / slice_dcm.max())
                    # slice_dcm = slice_dcm.astype("uint16")
                    slice_dcm = slice_dcm * ((pow(2, 8) - 1) / (slice_dcm.max() + 1e-8)) # t2 图像增强
                    slice_dcm = slice_dcm.astype("uint8") # 转换为uint8格式
                    slice_label = label[:, :, i]          # slice segmention label, four categories 0,1,2,4, 0 -> background

                    seg_label = np.copy(slice_label).astype("uint8")
                    # Need to be verify if label only 0,2
                    seg_label = seg_label * (255 / (seg_label.max() + 1e-8)) # segmentation 标签增强
                    # seg_label = seg_label * (255 / 4)
                    if not self.multi_label:
                        seg_label[seg_label > 0] = 1

                    if (np.count_nonzero(seg_label) < 10): # 有效标注过少，则跳过
                        continue
                    ...
                    # pdb.set_trace()
                    dcm_arr.append(slice_dcm) # dcm图像
                    label_arr.append(slice_label) # 头骨边缘
                    seg_arr.append(seg_label) # 分割label
        ...
        if self.is_test:
            ...
            add_seg_arr = []
            if factor >= 1:
                for i in range(len(dcm_arr)): # 对于每一层切片图像
                    dcm = dcm_arr[i]
                    times = 1
                    ...
                    skull_mask[dcm > 0] = 1
                    while times <= factor:
                        random_id = random.randrange(0, len(dcm_arr) - 1, 10) # 选取以10为因子的随机数作为id
                        seg = seg_arr[random_id]
                        # seg[seg > 0] = 1
                        seg = self.seg_in_skull(seg, skull_mask) # 取出segmentation区域的图像
    ...
    def merge_skull(self, skull_mask, slice_label, default_skull_value=5):
        ...
        skull_mask = ndimage.binary_fill_holes(skull_mask) # 孔洞填充
        skull_mask = cv2.Laplacian(skull_mask.astype("uint8"), cv2.CV_8U) # 边缘增强
